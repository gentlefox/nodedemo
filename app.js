...
passport.use(new LocalStrategy({ passReqToCallback: true },
  function(req, username, password, done) {
    ...
    // Not logged in.
    // Check for authentication via username and password to try to log in
    else if ((username.trim().length > 0) && (password.trim().length > 0)) {
      users.getByUsername(username, function(err, user) {
        if (err) {
          return done(err, false, { message: 'Authentication failed. Incorrect username or password.' });
        }

        if (!user) {
          return done(null, false, { message: 'Authentication failed. Incorrect username or password.' });
        }
...
// Checking if currrently authenticated
// Call at the start of every page to check for the user
